...
**Sync:** `useSubscriptions` inits from localStorage (instant), then fetches from DB on mount if authenticated (merges local-only entries). Mutations are async and optimistic — localStorage is updated first, then the DB call fires; on failure the local state rolls back and the error is thrown so the caller (form or list) can surface it to the user. `syncToDatabase` uses upsert + delete-not-in-list (not delete-all + insert) to avoid a transient empty-DB window. The "Save to cloud" button triggers `sync` (push then pull). Window focus and tab visibility change trigger a pull-only from DB (session-refreshed). The hook exposes `sync`, `syncing`, `lastSyncedAt`, and `isAuthenticated`.
...
**Sort & filter:** All sorting and filtering happens in `Dashboard.tsx` before passing `displayedSubscriptions` to `SubscriptionList`. Total monthly spend always uses the unfiltered `subscriptions` array; both pie charts use `displayedSubscriptions` so they reflect active filters. `SubscriptionList` renders in whatever order it receives — no internal sorting. Filters (category + payment method) live in a collapsible panel toggled by a "Filter" button that shows an active-count badge. Payment method filter row only renders when there are 2+ distinct methods.
